...
const ReTweetCard: React.FC<ReplyTweetState> = ({ reTweetData }) => {
  ...
  useEffect(() => {
    axios
      .get(UserURL + "/" + reTweetData.userId, {
        headers: {
  ...
  function handleTweetPageClick() {
    dispatch({
      type: "REPLY_DATA",
      payload: { tweetData: reTweetData, userData: replyedProfileData },
    });
    navigate(
          ...
          <div className="col-10">
            ...
            <div className="mt-2 ">
              <div className="cursor-pointer" onClick={handleTweetPageClick}>
                <p className="mb-2 p-0 ">{reTweetData.tweetContent}</p>
                {reTweetData.media.length > 0 &&
                  reTweetData.media.map((media: any) => {
